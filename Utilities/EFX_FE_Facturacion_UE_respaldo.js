 ...
 */
define(['N/record','N/search','N/runtime','./EFX_FE_Importe_en_Letra_ST','N/record','N/currency', 'N/config','./moment.js','N/query','N/file'],
    /**
     * @param{record} record
     */
    function(record,search,runtime,mod_monto_letra,modRecord,modcurrency, config,moment,query,file) {
            ...
            function beforeSubmit(context) {
                    ...
                    var clienteKiosko = newRec.getValue('custbody_efx_fe_kiosko_customer');
                        ...
                        var clienteObj = '';
                        if(clienteid){
                            if(recType != record.Type.PURCHASE_ORDER && recType != record.Type.ITEM_RECEIPT && recType != record.Type.VENDOR_BILL && recType !='customrecord_efx_fe_cp_carta_porte' && recType !='customrecord_efx_pagos_compensacion') {
                    ...
                    if(recType !='customrecord_efx_fe_cp_carta_porte' && recType !='customrecord_efx_pagos_compensacion') {
                        scriptdatossubsidiariabeforesubmit(context,clienteObj);
                    }
            }
            ...
            //DatosSubsidiaria.js
            function scriptdatossubsidiariabeforesubmit(context,clienteObjparam){
                                    ...
                                    try {
                                        ...
                                        if (custbody_efx_fe_comercio_exterior) {
                                            ...
                                            if (valAduana || valAduana == true || valAduana == 'true' || valAduana == 'T') {
                                                if (unitAduana) {
                                                    unitAduana = unitAduana.toFixed(3);
                                                    record.setSublistValue({
                                                        sublistId: 'item',
                            ...
                            for (var i = 0; i < line_count; i++) {
                                ...
                                for (var x = 0; x < tax_lines_count; x++) {
                                    ...
                                    for (var ret = 0; ret < config_retencion.length; ret++) {
                                        ...
                                        var rateresico = rate_number * (-1);
                                        
                                        if (tax_id == config_retencion[ret] && rateresico!=1.25) {
                                            ...
                                            if(hayieps==0) {
                                                if(montoIvas>0){
                                                    ...
                                                    if(hayret>1){
                                                        ...
                                                        tax_importe_resico = (rate_div * parseFloat(importe_base)) * base_calc;
                                                        log.audit({title:'tax_importe_resico',details:tax_importe_resico});
                                                    }else{
                                        ...
                                        }else if(tax_id == config_retencion[ret] && rateresico==1.25){
                                            //almacena la información del codigo de impuesto
                                            if(existeSuiteTax){
                                                obj_Json_Tax.resico.rate = rate_number;
                                            }else{
                                                obj_Json_Tax.resico.rate = rate_number * (-1);
                                            }

                                            obj_Json_Tax.resico.id = tax_id;
                                            obj_Json_Tax.resico.base = tax_base;
                                            obj_Json_Tax.resico.name = tax_name;
                                            //calcula el importe base con la base del impuesto y el importe de la linea
                                            // monto de la linea (amount) por la base de impuesto entre 100
                                            if(hayieps==0) {
                                                var base_imp = (parseFloat(importe_base) * parseFloat(tax_base)) / 100;
                                                obj_Json_Tax.resico.base_importe = base_imp.toFixed(2);
                                            }
                                            //se multiplica el rate number por -1 porque el impuesto de retencion se configura
                                            //en negativo la base
                                            if(existeSuiteTax){
                                            ...
                                            //se obtiene el rate del impuesto en decimales
                                            var rate_div = rate_num / 100;
                                            obj_Json_Tax.resico.rate_div = rate_div;
                                            if(hayieps==0) {
                                                if(montoIvas>0){
                                                    log.audit({title:'montoIvasresico',details:montoIvas});
                                                    log.audit({title:'importeDeImpuesto',details:importeDeImpuesto});
                                                    log.audit({title:'tax_importe_resico',details:tax_importe_resico});
                                                    if(tax_importe_resico!=0){
                                                        var tax_importe = montoIvas-importeDeImpuesto-tax_importe_resico;
                                                    }else{
                                                        var tax_importe = montoIvas-importeDeImpuesto;
                                                    }
                                            ...
                                            //se pone a 2 decimales el importe de impuesto
                                            obj_Json_Tax.resico.importe = tax_importe.toFixed(2);
                                            //tax_importe = parseFloat(tax_importe.toFixed(2));
                                ...
                                log.audit({title:'tax_lines_count',details:tax_lines_count});
                                    ...
                                    if(obj_Json_Tax.resico.name!=''){
                                        //var ajusteTotalesLimites = parseFloat(obj_Json_Tax_Limites.retenciones.importe)-parseFloat(obj_Json_Tax.retenciones.importe);
                                            importe_retencion = importe_retencion+parseFloat(obj_Json_Tax_Limites.resico.importe);
                                    }
                                    if(tax_lines_count>1){
            ...
            if (enviar_correos && destinatarios) {
