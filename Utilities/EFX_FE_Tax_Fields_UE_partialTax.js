...
define(['N/record','N/search','N/runtime'],
    ...
    function(record,search,runtime) {
        ...
        function afterSubmit(context){
            ...
            var partialTax = scriptObj.getParameter({ name: 'custscript_efx_fe_partial_tax' });
            log.audit({title:'partialTax',details:partialTax});
            ...
            try {
                if (context.type == context.UserEventType.CREATE || context.type == context.UserEventType.EDIT) {
                    ...
                    if (factEspejo != true || (factEspejo == true && tipogbl==2)) {
                        ...
                        if(partialTax){
                            var conteoVacios = 0;
                            var jsoncabeceraFull = record_now.getValue({fieldId:'custbody_efx_fe_tax_json'});
                            if(jsoncabeceraFull){
                                jsoncabeceraFull = JSON.parse(jsoncabeceraFull);
                            }
                        }
                        ...
                        for (var i = 0; i < line_count; i++) {
                            ...
                            var jsonlineafull = record_now.getSublistValue({
                                sublistId: 'item',
                                fieldId: 'custcol_efx_fe_tax_json',
                                line: i
                            });

                            if(jsonlineafull && partialTax){
                                continue;
                            }
                            if(partialTax){
                                conteoVacios++;
                                        ...
                                        if (tam_rates_iva > 0) {
                                            for (var t = 0; t < tam_rates_iva; t++) {
                                                if (Object.keys(obj_Json_Tax_totales.rates_iva)[t] == cadena_rate) {
                                                    ...
                                                    if(partialTax){
                                                        if(conteoVacios>0 && !jsoncabeceraFull){
                                                            ...
                                                            obj_Json_Tax_totales.rates_iva[cadena_rate] = (parseFloat(jsoncabeceraFull.rates_iva[cadena_rate]) + obj_iva_total).toFixed(2);
                                                            obj_Json_Tax_totales.rates_iva_data[rate_number] = (parseFloat(jsoncabeceraFull.rates_iva_data[rate_number]) + obj_iva_total).toFixed(2);
                                                            obj_Json_Tax_totales.bases_iva[rate_number] = parseFloat(jsoncabeceraFull.bases_iva[rate_number]) + obj_iva_total_base || 0;
                                                        }
                                                    }else{
                                                        obj_Json_Tax_totales.rates_iva[cadena_rate] = obj_iva_total.toFixed(2);
                                                        obj_Json_Tax_totales.rates_iva_data[rate_number] = obj_iva_total.toFixed(2);
                                                    ...
                                                    if (!obj_Json_Tax_totales.rates_iva[cadena_rate]) {
                                                        ...
                                                        if(partialTax){
                                                            if(conteoVacios>0 && !jsoncabeceraFull){
                                                                ...
                                                                obj_Json_Tax_totales.rates_iva[cadena_rate] = (parseFloat(jsoncabeceraFull.rates_iva[cadena_rate]) + tax_importe).toFixed(2);
                                                                obj_Json_Tax_totales.rates_iva_data[rate_number] = (parseFloat(jsoncabeceraFull.rates_iva_data[rate_number]) + tax_importe).toFixed(2);
                                                                obj_Json_Tax_totales.bases_iva[rate_number] = (parseFloat(jsoncabeceraFull.bases_iva[rate_number]) + base_imp) || '0.00';
                                                            }
                                                        }else{
                                                                obj_Json_Tax_totales.rates_iva[cadena_rate] = tax_importe.toFixed(2);
                                                                obj_Json_Tax_totales.rates_iva_data[rate_number] = tax_importe.toFixed(2);
                        ...
                        if(partialTax){
                            if(conteoVacios>0 && !jsoncabeceraFull){
                                ...
                                obj_Json_Tax_totales.subtotal = (parseFloat(jsoncabeceraFull.subtotal) + subtotalTransaccion).toFixed(2);
                            }
                        }else{
                            obj_Json_Tax_totales.subtotal = subtotalTransaccion.toFixed(2);
                        }
                        ...
                        if(partialTax){
                            if(conteoVacios>0 && !jsoncabeceraFull){
                                ...
                                if (descuentoglobal) {
                                    ...
                                    // descivatot = importe_iva*rate_desccabecera;
                                    // desciepstot = importe_ieps*rate_desccabecera;
                                    obj_Json_Tax_totales.total = (parseFloat(jsoncabeceraFull.total) + subtotalTransaccion + parseFloat(totalTraslados - totalRetenciones) - parseFloat(descuentoglobal) - parseFloat(desctraslados) - parseFloat(descret)).toFixed(2);
                                    obj_Json_Tax_totales.descuentoConImpuesto = (parseFloat(jsoncabeceraFull.descuentoConImpuesto) + parseFloat(descuentoglobal)).toFixed(2);
                                    obj_Json_Tax_totales.descuentoSinImpuesto = (parseFloat(jsoncabeceraFull.descuentoSinImpuesto) + parseFloat(descuentoglobal)).toFixed(2);
                                } else {
                                    obj_Json_Tax_totales.total = (parseFloat(jsoncabeceraFull.total) + subtotalTransaccion + parseFloat(totalTraslados - totalRetenciones) - parseFloat(obj_Json_Tax_totales.descuentoSinImpuesto)).toFixed(2);
                                }

                                obj_Json_Tax_totales.totalImpuestos = (parseFloat(jsoncabeceraFull.totalImpuestos) + parseFloat(totalTraslados - totalRetenciones - desctraslados - descret)).toFixed(2);
                                obj_Json_Tax_totales.totalTraslados = (parseFloat(jsoncabeceraFull.totalTraslados) + totalTraslados - desctraslados).toFixed(2);
                                obj_Json_Tax_totales.totalRetenciones = (parseFloat(jsoncabeceraFull.totalRetenciones) + totalRetenciones - descret).toFixed(2);

                                //se llena el objeto con los totales de impuestos por tipo
                                obj_Json_Tax_totales.retencion_total = (parseFloat(jsoncabeceraFull.retencion_total) + parseFloat(importe_retencion - descret)).toFixed(2) || '0.00';
                                obj_Json_Tax_totales.iva_total = (parseFloat(jsoncabeceraFull.iva_total) + parseFloat(importe_iva - descivatot)).toFixed(2) || '0.00';
                                obj_Json_Tax_totales.exento_total = (parseFloat(jsoncabeceraFull.exento_total) + parseFloat(importe_exento)).toFixed(2) || '0.00';
                                obj_Json_Tax_totales.ieps_total = (parseFloat(jsoncabeceraFull.ieps_total) + parseFloat(importe_ieps - desciepstot)).toFixed(2) || '0.00';
                                obj_Json_Tax_totales.local_total = (parseFloat(jsoncabeceraFull.local_total) + parseFloat(importe_local)).toFixed(2) || '0.00';
                            }
                        }else {
                            if (descuentoglobal) {
                                // desctraslados = totalTraslados*rate_desccabecera;
            ...
            for(var st=0;st<countimpuesto;st++){
                ...
                if(taxref_imp==taxref_linea){
                    if(objST.hasOwnProperty(taxref_imp)){
                        ...
                        objLineaSt.taxcode = record_now.getSublistValue({sublistId:'taxdetails',fieldId:'taxcode',line:st});
                        objLineaSt.rate = record_now.getSublistValue({sublistId:'taxdetails',fieldId:'taxrate',line:st});
                        objLineaSt.nombre = record_now.getSublistValue({sublistId:'taxdetails',fieldId:'taxcode_display',line:st});
